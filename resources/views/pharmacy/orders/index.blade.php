...
@section('page-title', 'طلبات الأدوية')
...
<div class="mb-6 flex flex-wrap gap-3 items-center">
    <button onclick="filterStatus('new')" data-filter="new" class="filter-btn px-4 py-2 rounded-lg bg-emerald-100 text-emerald-800 font-semibold text-sm">جديدة <span class="count-badge hidden ml-1 bg-emerald-600 text-white text-xs px-1.5 rounded-full"></span></button>
    ...
    <button onclick="filterStatus('preparing')" data-filter="preparing" class="filter-btn px-4 py-2 rounded-lg bg-gray-100 text-gray-700 font-semibold text-sm">قيد التجهيز</button>
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b">
                <tr>
                    ...
                    <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">التنفيذ</th>
                    <th class="px-6 py-3 text-right text-sm font-semibold text-gray-700">الأدوية</th>
            ...
            <tbody id="ordersBody">
                <tr><td colspan="8" class="px-6 py-8 text-center text-gray-500">جاري التحميل...</td></tr>
            </tbody>
        </table>
    </div>
</div>
...
function filterStatus(status, reload = true) {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        ...
        btn.className = active
            ? 'filter-btn px-4 py-2 rounded-lg bg-emerald-100 text-emerald-800 font-semibold text-sm'
...
async function loadOrderStats() {
    ...
    document.getElementById('orderStats').innerHTML = `
        ...
        <div class="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
            <p class="text-xs text-emerald-700">نشطة إجمالاً</p>
            <p class="text-xl font-bold text-emerald-800">${m.active_orders}</p>
        </div>
    `;
...
async function loadOrders() {
    ...
    const data = await apiCall(`/pharmacy/api/orders?${params}`);
    const tbody = document.getElementById('ordersBody');
    if (!data?.success || !data.data?.orders?.length) {
        tbody.innerHTML = '<tr><td colspan="8" class="px-6 py-8 text-center text-gray-500">لا توجد طلبات</td></tr>';
        return;
    }
        ...
        <tr class="border-b hover:bg-gray-50">
            ...
            <td class="px-6 py-4 text-sm">
                ${o.has_prescription_image || o.source === 'prescription'
                    ...
                    : '<span class="text-emerald-700"><i class="fas fa-list ml-1"></i>كتالوج</span>'}
            </td>
            <td class="px-6 py-4 text-sm">
                ${o.fulfillment_type === 'delivery'
                    ? '<span class="text-blue-700"><i class="fas fa-truck ml-1"></i>' + (o.fulfillment_label || 'توصيل') + '</span>'
                    : '<span class="text-gray-700"><i class="fas fa-store ml-1"></i>' + (o.fulfillment_label || 'استلام') + '</span>'}
            </td>
            ...
            <td class="px-6 py-4">
                <a href="/pharmacy/dashboard/orders/${o.id}" class="px-3 py-1 bg-emerald-600 text-white text-xs rounded-lg hover:bg-emerald-700">عرض</a>
            </td>
        </tr>
    `).join('');
}
