    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form id="subscribeForm" onsubmit="submitSubscription(event)" class="p-6 space-y-5">
            ...
            <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                <p class="text-sm text-emerald-800 font-semibold mb-2">معلومات الدفع</p>
            ...
            <button type="submit" class="w-full bg-emerald-600 text-white py-3 rounded-lg font-semibold hover:bg-emerald-700">
                إرسال طلب الاشتراك
            </button>
        </form>
    </div>
...
async function loadSubscriptionStatus() {
    const data = await apiCall('/pharmacy/api/subscription');
...
async function loadPlans() {
    const data = await apiCall('/pharmacy/api/subscription/plans');
    ...
    if (!data.data.length) {
        grid.innerHTML = '<p class="text-gray-500 col-span-3">لا توجد باقات صيدليات متاحة حالياً. تواصل مع الإدارة.</p>';
        return;
    }
    ...
    grid.innerHTML = data.data.map(plan => `
        <div class="bg-white rounded-xl shadow-sm p-6 border ${plan.is_featured ? 'border-amber-400 ring-2 ring-amber-100' : ''}">
            ...
            <p class="text-2xl font-bold text-emerald-600 my-3">${formatCurrency(plan.price)}</p>
            ...
            <button onclick="openSubscribeModal(${plan.id}, '${plan.name.replace(/'/g, "\\'")}', ${plan.price})"
                class="w-full py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 ${plan.price <= 0 ? 'opacity-50 cursor-not-allowed' : ''}"
...
async function loadPaymentSettings() {
    const data = await apiCall('/pharmacy/api/payment-settings');
...
async function submitSubscription(e) {
    ...
    try {
        const data = await apiCall('/pharmacy/api/subscription/subscribe', { method: 'POST', body: formData });
        if (data?.success) {
            closeSubscribeModal();
            alert(data.message);
