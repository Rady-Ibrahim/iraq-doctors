    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form id="branchForm" onsubmit="saveBranch(event)" class="p-6 space-y-4">
            ...
            <label class="flex items-center gap-2">
                <input type="checkbox" id="branch_is_primary" class="rounded text-indigo-600">
...
async function loadBranches() {
    ...
    list.innerHTML = data.data.map(b => `
        <div class="bg-white rounded-xl shadow-sm p-5 border ${b.is_primary ? 'border-indigo-300' : ''}">
            <div class="flex justify-between items-start mb-2">
                <h4 class="font-bold text-gray-800">${b.branch_name}${b.is_primary ? ' <span class="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded">رئيسي</span>' : ''}</h4>
                <div class="flex gap-2">
                    <button onclick='editBranch(${JSON.stringify(b)})' class="text-indigo-600 text-sm">تعديل</button>
        ...
        return `
            <div class="flex flex-wrap items-center gap-2 border rounded-lg p-2">
                <label class="flex items-center gap-2 w-24">
                    <input type="checkbox" data-day="${day}" class="bwh-enabled rounded text-indigo-600" ${h.enabled ? 'checked' : ''}>
