...
<body class="bg-gradient-to-br from-indigo-50 to-violet-100 min-h-screen flex items-center justify-center">
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-4">
        ...
        <form method="POST" action="{{ route('laboratory.verify-phone.send') }}" class="mb-4">
            @csrf
            <button type="submit"
        ...
        <form method="POST" action="{{ route('laboratory.verify-phone.submit') }}" class="space-y-4">
            @csrf
            <div>
            ...
            <button type="submit"
                class="w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white py-3 rounded-lg font-semibold hover:from-indigo-700 hover:to-violet-700 transition">
                تأكيد التفعيل
            </button>
        </form>
