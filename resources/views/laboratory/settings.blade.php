...
@section('title', 'إعدادات المختبر')
@section('page-title', 'إعدادات المختبر')
@section('page-description', 'الملف الشخصي، ساعات العمل، وسحب العينات من المنزل')

@section('content')
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-6">معلومات المختبر</h3>
        <form id="profileForm" onsubmit="saveProfile(event)" class="space-y-4" enctype="multipart/form-data">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">اسم المختبر</label>
                <input type="text" id="name" name="name" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">
            ...
            <button type="submit" class="w-full py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">حفظ الإعدادات</button>
        </form>
    </div>

    <div class="space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">سحب عينات من المنزل</h3>
            <label class="flex items-center gap-2 mb-4">
                <input type="checkbox" id="home_collection_enabled" class="rounded text-indigo-600">
                <span class="text-sm text-gray-700">تفعيل خدمة السحب من المنزل</span>
            </label>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">رسوم السحب (د.ع)</label>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <button type="button" onclick="saveWorkingHours()" class="mt-4 w-full py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">حفظ ساعات العمل</button>
        </div>
    </div>
</div>
@endsection
...
async function loadProfile() {
    const data = await apiCall('/laboratory/api/profile');
    ...
    document.getElementById('home_collection_enabled').checked = !!profileData.home_collection_enabled;
    document.getElementById('home_collection_fee').value = profileData.home_collection_fee || '';
    if (profileData.logo) {
...
function renderWorkingHours(hours) {
    ...
    container.innerHTML = Object.keys(dayLabels).map(day => {
        const h = hours[day] || { enabled: day !== 'friday', open: '08:00', close: '20:00' };
        return `
            <div class="flex flex-wrap items-center gap-2 border rounded-lg p-3">
                <label class="flex items-center gap-2 w-28">
                    <input type="checkbox" data-day="${day}" class="wh-enabled rounded text-indigo-600" ${h.enabled ? 'checked' : ''}>
                ...
                <input type="time" data-day="${day}" class="wh-close px-2 py-1 border rounded text-sm" value="${h.close || '20:00'}">
            </div>`;
    }).join('');
}
...
function collectWorkingHours() {
    ...
    Object.keys(dayLabels).forEach(day => {
        hours[day] = {
            ...
            close: document.querySelector(`.wh-close[data-day="${day}"]`)?.value || '20:00',
        };
    });
    return hours;
}
...
async function saveProfile(e) {
    ...
    formData.append('home_collection_enabled', document.getElementById('home_collection_enabled').checked ? '1' : '0');
    formData.append('home_collection_fee', document.getElementById('home_collection_fee').value || '0');
    formData.append('working_hours', JSON.stringify(collectWorkingHours()));
    try {
        const data = await apiCall('/laboratory/api/profile', { method: 'POST', body: formData });
