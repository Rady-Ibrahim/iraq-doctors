...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-6">
    <div class="bg-white rounded-2xl shadow-lg max-w-lg w-full p-8">
        ...
        @if ($laboratory->reject_reason)
            <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <p class="text-sm font-semibold text-red-800 mb-1">سبب الرفض:</p>
                <p class="text-red-700">{{ $laboratory->reject_reason }}</p>
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('laboratory.resubmit') }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
        ...
        <form method="POST" action="{{ route('laboratory.logout') }}" class="mt-4">
            @csrf
