    ...
    <form id="supportAddForm" onsubmit="supportSave(event)"
        ...
        <div>
            ...
            <input type="text" id="svc_name" required placeholder="مثل: الدعم الفني"
                class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-400">
        </div>
        <div>
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form id="supportEditForm" onsubmit="supportUpdate(event)" class="space-y-3">
            ...
            <div class="flex gap-3 pt-1">
                <button type="submit" class="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm">حفظ</button>
...
async function loadSupportContacts() {
    const data = await apiCall('/laboratory/api/support-contacts');
    ...
    tbody.innerHTML = supportItems.map(i => `
        <tr class="border-b hover:bg-gray-50">
            ...
            <td class="px-5 py-3 font-mono">${i.call_phone
                ? `<a href="tel:${i.call_phone}" class="text-indigo-600 hover:underline">${esc(i.call_phone)}</a>`
                : '<span class="text-gray-300">—</span>'}</td>
...
async function supportSave(e) {
    e.preventDefault();
    const name=document.getElementById('svc_name').value.trim(), whatsapp=document.getElementById('svc_whatsapp').value.trim(), call=document.getElementById('svc_call').value.trim();
    if (!whatsapp&&!call){alert('أدخل رقم واتساب أو رقم اتصال');return;}
    const data=await apiCall('/laboratory/api/support-contacts',{method:'POST',body:JSON.stringify({service_name:name,whatsapp_phone:whatsapp||null,call_phone:call||null,is_active:true})});
    if(data?.success){showSuccess(data.message||'تم');document.getElementById('supportAddForm').reset();loadSupportContacts();}else handleApiError(data);
}
function supportOpenEdit(id){const item=supportItems.find(i=>i.id===id);if(!item)return;document.getElementById('svc_edit_id').value=item.id;document.getElementById('svc_edit_name').value=item.service_name||'';document.getElementById('svc_edit_whatsapp').value=item.whatsapp_phone||'';document.getElementById('svc_edit_call').value=item.call_phone||'';const m=document.getElementById('supportEditModal');m.classList.remove('hidden');m.classList.add('flex');}
...
async function supportUpdate(e){e.preventDefault();const id=document.getElementById('svc_edit_id').value;const data=await apiCall(`/laboratory/api/support-contacts/${id}`,{method:'PUT',body:JSON.stringify({service_name:document.getElementById('svc_edit_name').value.trim(),whatsapp_phone:document.getElementById('svc_edit_whatsapp').value.trim()||null,call_phone:document.getElementById('svc_edit_call').value.trim()||null})});if(data?.success){showSuccess(data.message||'تم');supportCloseEdit();loadSupportContacts();}else handleApiError(data);}
async function supportToggle(id,isActive){const data=await apiCall(`/laboratory/api/support-contacts/${id}`,{method:'PUT',body:JSON.stringify({is_active:!isActive})});if(data?.success)loadSupportContacts();}
async function supportDelete(id){if(!await confirmAction('حذف؟'))return;const data=await apiCall(`/laboratory/api/support-contacts/${id}`,{method:'DELETE'});if(data?.success){showSuccess('تم الحذف');loadSupportContacts();}}
