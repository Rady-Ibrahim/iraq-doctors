    ...
    <form id="supportAddForm" onsubmit="supportSave(event)"
        ...
        <div>
            ...
            <input type="text" id="svc_name" required placeholder="مثل: الدعم الفني"
                class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-teal-400">
        </div>
        <div>
        ...
        <div class="sm:col-span-3 flex items-center gap-3">
            <button type="submit"
                class="px-5 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition text-sm">
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form id="supportEditForm" onsubmit="supportUpdate(event)" class="space-y-3">
            ...
            <div class="flex gap-3 pt-1">
                <button type="submit" class="flex-1 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 text-sm">حفظ</button>
...
async function loadSupportContacts() {
    const data = await apiCall('/doctor/api/support-contacts');
    ...
    tbody.innerHTML = supportItems.map(i => `
        <tr class="border-b hover:bg-gray-50">
            ...
            <td class="px-5 py-3 font-mono">${i.call_phone
                ? `<a href="tel:${i.call_phone}" class="text-teal-600 hover:underline">${esc(i.call_phone)}</a>`
                : '<span class="text-gray-300">—</span>'}</td>
            <td class="px-5 py-3">
            ...
            <td class="px-5 py-3">
                <div class="flex gap-1 flex-wrap">
                    <button onclick="supportOpenEdit(${i.id})" class="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs hover:bg-gray-200">تعديل</button>
                    <button onclick="supportToggle(${i.id},${i.is_active})"
                        class="px-2 py-1 rounded text-xs ${i.is_active ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}">
...
async function supportSave(e) {
    ...
    if (!whatsapp && !call) { alert('أدخل رقم واتساب أو رقم اتصال على الأقل'); return; }
    const data = await apiCall('/doctor/api/support-contacts', {
        method: 'POST',
        body: JSON.stringify({ service_name: name, whatsapp_phone: whatsapp || null, call_phone: call || null, is_active: true }),
    });
    if (data?.success) { showSuccess(data.message || 'تم الإضافة'); document.getElementById('supportAddForm').reset(); loadSupportContacts(); }
...
async function supportUpdate(e) {
    ...
    const data = await apiCall(`/doctor/api/support-contacts/${id}`, {
        method: 'PUT',
        body: JSON.stringify({
    ...
    if (data?.success) { showSuccess(data.message || 'تم التحديث'); supportCloseEdit(); loadSupportContacts(); }
    else { handleApiError(data); }
}

async function supportToggle(id, isActive) {
    const data = await apiCall(`/doctor/api/support-contacts/${id}`, {
...
async function supportDelete(id) {
    if (!await confirmAction('حذف خدمة الدعم هذه؟')) return;
    const data = await apiCall(`/doctor/api/support-contacts/${id}`, { method: 'DELETE' });
    if (data?.success) { showSuccess('تم الحذف'); loadSupportContacts(); }
}
