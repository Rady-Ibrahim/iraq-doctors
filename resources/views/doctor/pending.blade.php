...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-6">
    <div class="bg-white rounded-2xl shadow-lg max-w-lg w-full p-8 text-center">
        ...
        <h1 class="text-2xl font-bold text-gray-800 mb-3">حسابك قيد المراجعة</h1>
        <p class="text-gray-600 mb-6">
            تم استلام طلب التسجيل بنجاح. فريق الإدارة يراجع مستنداتك وسيتم إشعارك عند الموافقة.
        </p>
        @if (session('success'))
        ...
        <div class="bg-gray-50 rounded-lg p-4 text-right mb-6">
            <p class="text-sm text-gray-600">الاسم: <span class="font-semibold text-gray-800">{{ auth()->user()->name }}</span></p>
            <p class="text-sm text-gray-600 mt-1">التخصص: <span class="font-semibold text-gray-800">{{ $doctor->speciality?->name_ar ?? '-' }}</span></p>
