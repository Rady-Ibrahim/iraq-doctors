...
<body class="bg-gradient-to-br from-teal-50 to-cyan-100 min-h-screen flex items-center justify-center">
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-4">
        ...
        <form method="POST" action="{{ route('doctor.verify-phone.send') }}" class="mb-4">
            @csrf
            <button type="submit"
        ...
        <form method="POST" action="{{ route('doctor.verify-phone.submit') }}" class="space-y-4">
            @csrf
            <div>
                ...
                <input type="text" name="code" maxlength="6" inputmode="numeric" autocomplete="one-time-code" required
                    value="{{ old('code') }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 text-center text-2xl tracking-widest @error('code') border-red-500 @enderror"
                    placeholder="000000">
            </div>
            <button type="submit"
