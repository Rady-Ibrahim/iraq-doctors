...
@section('title', 'إدارة الصيدليات')
@section('page-title', 'الصيدليات')
@section('page-description', 'مراجعة طلبات تسجيل الصيدليات والموافقة عليها')

@section('content')
<div class="bg-white rounded-xl shadow-sm p-6 mb-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <input type="text" id="searchInput" placeholder="ابحث عن صيدلية..." class="w-full px-4 py-2 border rounded-lg">
        ...
        <button onclick="loadPharmacies()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">بحث</button>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <table class="w-full">
        <thead class="bg-gray-50 border-b">
            <tr>
                <th class="px-6 py-3 text-right text-sm font-semibold">الصيدلية</th>
...
async function loadPharmacies(page = 1) {
    ...
    const data = await apiCall(`/admin/api/pharmacies?${params}`);
    ...
    tbody.innerHTML = data.data.map(pharmacy => `
        <tr class="border-b hover:bg-gray-50">
            <td class="px-6 py-4 font-semibold">${pharmacy.name || '-'}</td>
            <td class="px-6 py-4">${pharmacy.owner_name || '-'}</td>
            <td class="px-6 py-4">${pharmacy.governorate || '-'}</td>
            <td class="px-6 py-4">${statusBadge(pharmacy.status)}</td>
            <td class="px-6 py-4">
                <a href="/admin/dashboard/pharmacies/${pharmacy.id}" class="text-blue-600 hover:underline">عرض</a>
            </td>
        </tr>
    `).join('');
}
