...
use Modules\Pharmacy\Http\Requests\Api\CreatePharmacyOrderRequest;
...
class PharmacyOrderController extends Controller
{
    ...
    public function __construct(private PharmacyOrderService $orderService) {}
    ...
    protected function ensurePatient(): ?JsonResponse
    {
        ...
        if (! $user?->isPatient()) {
            return $this->forbidden('فقط المرضى يمكنهم إدارة طلبات الصيدلية', 'NOT_PATIENT');
        }

        return null;
    }

    public function store(CreatePharmacyOrderRequest $request): JsonResponse
    {
