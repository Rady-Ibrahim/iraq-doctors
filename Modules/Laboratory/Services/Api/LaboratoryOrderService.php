...
class LaboratoryOrderService
{
    public function createOrder(
        ...
        return DB::transaction(function () use ($patientId, $data, $prescriptionImage) {
            ...
            if (! $hasItems && ! $hasImage && ! $prescriptionRecord) {
                throw new \InvalidArgumentException('يجب اختيار تحاليل أو إرفاق صورة روشتة أو ربط روشتة الطبيب');
            }
            ...
            $order = LaboratoryOrder::create([
                ...
                'collection_type'        => $data['collection_type'] ?? 'clinic',
                'patient_address'        => ($data['collection_type'] ?? 'clinic') === 'home' ? ($data['patient_address'] ?? null) : null,
                'patient_latitude'       => ($data['collection_type'] ?? 'clinic') === 'home' ? ($data['patient_latitude'] ?? null) : null,
                'patient_longitude'      => ($data['collection_type'] ?? 'clinic') === 'home' ? ($data['patient_longitude'] ?? null) : null,
            ...
            return $order->fresh(['laboratory.governorate', 'items', 'patient', 'prescriptionRecord.doctor.user']);
        });
    }
    ...
    {
        return LaboratoryOrder::with(['laboratory.governorate', 'items', 'results', 'branch', 'prescriptionRecord.doctor.user'])
            ->where('patient_id', $patientId)
            ->find($orderId);
    }
    ...
    public function formatOrderForPatient(LaboratoryOrder $order, bool $detailed = false): array
    {
        ...
        if ($detailed) {
            ...
            $data += [
                ...
                'collection_label'    => $order->collection_type === 'home' ? 'جمع منزلي' : 'مراجعة العيادة',
                'patient_address'     => $order->patient_address,
                'patient_latitude'    => $order->patient_latitude,
                'patient_longitude'   => $order->patient_longitude,
